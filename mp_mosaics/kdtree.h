...
class KDTree
{
  ...
  private:
    ...
    double distance(const Point<Dim> &firstpoint, const Point<Dim> &secondpoint) const;

    KDTreeNode* newtree(vector<Point<Dim>> &list, int components, unsigned left, unsigned right);

    Point<Dim> quickselect(vector<Point<Dim>> &list, int components, unsigned left, unsigned right, unsigned var);

    unsigned partition(vector<Point<Dim>> &list, int components, unsigned left, unsigned right, unsigned indexvar);

    void swap(Point<Dim> &firstpoint, Point<Dim> &secondpoint);

    void copy(KDTree<Dim> *sub1, KDTree<Dim> *sub2);

    void clear(KDTreeNode *&node);

    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode *node, int components) const;
